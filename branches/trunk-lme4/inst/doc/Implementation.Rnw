...
conveniently we change the representation use a matrix/vector
...
The columns of Z are indicator columns (that is, the $i$th column has
...
parameter vector determining $\bm{\Sigma}(\bm{\theta})$, which can be
...
information needed to evaluate the ML deviance or the REML as a
function of $\bm{\theta}$.  The components of the deviance are given
in the \code{deviance} slot of the fitted model
<<devRm1ML>>=
Rm1ML@deviance
@
The element labelled \code{ldZ} is the logarithm of the square
determinant of the upper left $6\times 6$ section of $\bm{L}$.  This
corresponds to $\log\left|{\bm{Z}^*}\trans\bm{Z}^*+\bm{I}_q\right|$
where $\bm{Z}^*=\bm{Z}\bm{T}\bm{S}$.  We can verify that this value
can be calculated in this way by
<<ldZRm1ML>>=
2*6*log(as(Rm1ML@L, "sparseMatrix")[1,1])
all.equal(2*6*log(as(Rm1ML@L, "sparseMatrix")[1,1]),
 unname(Rm1ML@deviance["ldZ"]))
@ 
