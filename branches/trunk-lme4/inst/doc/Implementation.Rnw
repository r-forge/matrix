...
terms of $\bSigma^{-1}$, which is called the \emph{precision} matrix
of the random effects, than in terms of $\bSigma$, the
variance-covariance matrix.  In fact, the formulae are most
conveniently expressed in terms of the \emph{relative precision
...
non-redundant elements in the $\bOmega_i$.  In fact we use a
different, but equivalent, parameterization for reasons to be
discussed later.
...
with Jacobian
\[
\left|\frac{d\bv}{d\bb}\right|=\left||\bL\bP|\right|=
