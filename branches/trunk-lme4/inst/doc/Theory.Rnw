...
distribution as $f_{\bm Y|\bm B}$, which will mean the condition probability
density or the conditional probability
mass function, whichever is appropriate.  This conditional
...
conditional mean $\bm\mu_{\bm Y|\bm B}$ and the observed data $\bm y$.
For linear mixed models and for nonlinear mixed models $d(\bm\mu_{\bm
  Y|\bm B},\bm y)$ is exactly the square of the Euclidean distance,
$d(\bm\mu_{\bm Y|\bm B},\bm y)=\left\|\bm\mu_{\bm Y|\bm B}-\bm
  y\right\|^2$.  The more general form of the discrepancy function
used for generalized linear mixed models is described in
\S\ref{sec:GLMM}.  The general form of the conditional distribution is
\begin{equation}
  \label{eq:conddens}
...
In the models we will consider the elements of $\bm Y$ are modelled as
being conditionally independent, given $\bm B$.  That is, $f_{\bm
  Y|\bm B}$ can be written as a product of $n$ factors, each involving
just one element of $\bm y$ and the corresponding element of
$\bm\mu_{\bm Y|\bm B}$.  Consequently, the discrepancy can be evaluated
as a sum of $n$ terms evaluated component-wise from
$\bm\mu(\bm\eta_{\bm b}(\bm b,\bm\beta))$ and $\bm y$.
...
$\bm\eta$ is a multiple, $m=ns$, of $n$, the dimension of $\bm\mu$, we
convert $\bm\eta$ to an $n\times s$ matrix (using, say, column-major
ordering) and the restriction in this case is that the $i$th element of
$\bm\mu$ depends only on the $i$th row of the $\bm\eta$ matrix.

Generalized linear mixed models where the independent components of the
conditional distribution, $f_{\bm Y|\bm B}$, have Bernoulli
distributions or binomial distributions or Poisson distributions, do
not have a variance scale parameter because these conditional
distributions are completely determined by the conditional mean.  In
such cases the conditional distribution can be written $f_{\bm Y|\bm
  B}(\bm y|\bm b,\bm\beta)= k(\bm y)e^{-d(\bm\mu_{\bm Y|\bm B}(\bm
  b,\bm\beta)),\bm y)/2}$.

The normalization factor, $k(\bm y,\sigma^2)$, (or $k(\bm y)$ when
there is no scale factor), does not depends on the conditional mean.
...
(\ref{eq:conddens}) and $\bm\Sigma(\bm\theta)$ is a $q\times q$
symmetric, positive-semidefinite matrix, called the \emph{relative
  variance-covariance matrix} of the random effects.  The form of
$\bm\Sigma(\bm\theta)$ and the parameter, $\bm\theta$, that determines
$\bm\Sigma$ are described in \S\ref{sec:relvarcov}. (The condition that
...
the $m\times q_i$ model matrix $\bm Z_i$ according to the usual rules
for model matrices, in the case of linear or generalized linear
models, and according to slightly modified rules, described in
...
@
The corresponding indicator interaction matrix, $\tilde{\bm Z}_i$,
which is simply the matrix of indicator columns of the levels of
\code{subj}, is
...
The transpose of the indicator matrix is generated by coercing a
factor to the virtual class \code{"sparseMatrix"}
<<extendedMM1t>>=
as(dat$subj, "sparseMatrix")
@
The rows of the transpose are associated with the levels of the
grouping factor.
...
dat$time <- rep(1:3, 4)
@
Now the first few rows of the data frame are
<<dathead>>=
head(dat, n = 5)
@
...
columns, whose first few rows are
<<Z1mat>>=
head(Zi <- model.matrix(~ time, dat), n = 5)
@
The transpose of the indicator interaction matrix, $\tilde{\bm
  Z}_i\trans$, could be constructed as
<<Z1tilde>>=
