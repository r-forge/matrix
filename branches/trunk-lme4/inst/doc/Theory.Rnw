...
\begin{displaymath}
  f_{\bm U|\bm Y}(\bm u|\bm u,\bm\beta,\bm\theta,\sigma^2)\propto
...
depends on $\sigma^2$ in addition to the other parameters but the
conditional mode does not.)
...
of the integrand that will depend on $\bm{u}$ is the quadratic
term in the exponent.  To evaluate the non-constant part of the
integral, which we can write as
\begin{displaymath}
...
\begin{equation}
  \label{eq:vLu}
  \bm{v}=\bm L(\bm\beta,\bm\theta,\bm y)\trans(\bm{u}-\tilde{\bm{u}})/\sigma .
\end{equation}
The determinant of the Jacobian of the transformation (\ref{eq:vLu}) is
\begin{displaymath}
  \left|\frac{d\bm{v}}{d\bm{u}}\right|=\frac{|\bm L(\bm\beta,\bm\theta,\bm y)|}{\sigma^q} .
\end{displaymath}
implying that the differential, $d\bm u$, is to be replaced by
\begin{displaymath}
...
\begin{equation}
  ...
  =\left|\bm L(\bm\beta,\bm\theta,\bm y)\right|^{-1} ,
\end{equation}
because the integral of the density function over $\mathbb{R}^q$ must be unity.
...
%% Perhaps move this paragraph to the section on linear mixed models?

In both models the conditional distribution of the response, given the
random effects, is a ``spherical'' Gaussian, $\bm Y|\bm
...
\end{align}
(The distribution $\mathcal{N}(\bm\mu,\sigma^2\bm I)$ is said to be
``spherical Gaussian'' because contours of its density are spheres in
$\mathbb{R}^n$.)

For a nonlinear mixed model, however, the mean function,
$\bm\mu(\bm\eta)$, is not the identity, as it is for the linear mixed
model.  In the nonlinear form each element of $\bm\mu$ is the value of
a scalar nonlinear model function, $g(\bm x,\bm\phi)$, that depends on
the observed values of some covariates, $\bm{x}$, and on a parameter
vector, $\bm\phi$, of length $s$.  This model function, $g(\bm
x,\bm\phi)$, can be nonlinear in some or all of the elements of
$\bm\phi$.

When fitting a model the values of the covariates at each observation
are known so we can regard the $i$th element of $\bm\mu_{\bm Y|\bm B}$
as a function of $\bm\phi_i$ only and write
\begin{equation}
  \label{eq:vectorg}
  \bm\mu=\bm g(\bm\Phi)
\end{equation}
...
rows of $\bm\Phi$ and corresponding covariates $\bm{x}_i,i=1,\dots,n$.
...
The matrix $\bm\Phi$ is $n\times s$ hence the dimension of
...
conditional mean $\bm\mu_{\bm Y|\bm U}$
is a linear function of $\bm u$ (but a nonlinear function of one or
more components of $\bm\beta$, so the model cannot be written as a
linear mixed model).  For these \emph{conditionally linear mixed
  models} (\ref{eq:nmmLinApprox}) is exact and $\bm M$ does not depend
upon $\bm u$.
...
\end{equation}
The increment $\bm\delta^{(i)}$ is evaluated by solving the two sparse
triangular systems in
\begin{equation}
...
penalized least squares problem for the updated random effects,
$\bm u^{(i+1)}=\bm u^{(i)}+\bm\delta^{(i)}$ directly.  We prefer to
write the conditions in terms of the increment and to calculate the
proposed increment $\bm\delta^{(i)}$ for two reasons: to allow us to
incorporate a step factor \citep[\S2.2.1]{bateswatts88:_nonlin} easily
and to evaluate the relative offset convergence criterion based on
orthogonality of the residual vector to the columns of the gradient
matrix.

With highly nonlinear models it can happen that the proposed update actually
increases the penalized discrepancy rather than decreasing it.  In
these cases we use only a fraction $h$ of the proposed step where
...
factor is reached.  Generally a simple strategy starting at $h=1$ each
iteration and successively halving $h$ if necessary is sufficient.
