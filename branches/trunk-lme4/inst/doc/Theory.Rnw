...
\section{Specific types of mixed models}
\label{sec:mixedTypes}
...
\subsubsection{Optimizing the penalized discrepancy}
...
penalized discrepancy function in a nonlinear mixed model can be
written as a penalized least squares problem
\begin{equation}
  \label{eq:nlmmdisc}
...
For some models (\ref{eq:nlmmdisc}) is a penalized linear least
squares problem with a direct solution but, in general, it must be
considered as a penalized nonlinear least squares problem that
