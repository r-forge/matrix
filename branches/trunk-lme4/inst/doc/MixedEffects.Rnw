...
scores of 134,713 students in 3722 schools.  There are 377,111
elements in the pairwise crosstabulation (138435 diagonal elements and
238,676 off-diagonal elements).  The projection of the $(2,1)$ block
into the $(2,2)$ block produces a symmetric $3722\times 3722$ matrix
with  49,305 non-redundant elements (3722 diagonals and 45,583
...
number of off-diagonals is reduced to 193,562 or 2.8\% of the maximum.
...
\begin{equation}
  ...
  \boeta = \bX\bbeta+\bZ\bb
\end{equation}
is transformed through a link function $g$ to produce the vector of
predicted means
\begin{equation}
  \label{eq:GLMMmean}
  \bmu = g(\bX\bbeta+\bZ\bb)
\end{equation}
and the conditional density of the responses,
$p(\by|\bbeta,\bb,\btheta)$ may be other than Gaussian.

Usually the conditional density is a member of the exponential family,
such as the Bernoulli distribtution for binary responses or Poisson
for count data, in which case there is a canonical link function,
such as the logit link for the Bernoulli and the log link for the Poisson.
