...
\begin{abstract}
  ...
  sparse matrix.  This representation provides for efficient
  ...
  matrix have a special structure in the linear mixed-effects problem
...
each of the $\bZ_i,i=1,\dots,k$ consist of a single column of 1's.
...
function to perform the symbolic analysis and another function that
...
$\bZ\trans\bZ$, as suggested in \citep{MLwiN:2002}, would require 2812
...
than the fill-reducing permutation, the reduces both the space and the
...
permutation can be substantial.  We have used these
methods to analyze 378,047 test scores of 134,713 students in 3722
schools.  There are 377,111 nonredundant nonzeros in the pairwise crosstabulation
(138435 diagonals and 238,676 off-diagonals).  The
projection of the $(2,1)$ block into the $(2,2)$ block produces a
symmetric $3722\times 3722$ matrix with 49,305 non-redundant nonzeros
(3722 diagonals and 45,583 off-diagonals).  Without the fill-reducing
permutation the $\bL$ matrix has 4,469,124 nonzero off-diagonals or
64.5\% of the maximum possible number (6,924,781). With the
permutation the number of nonzero off-diagonals is reduced to 193,562 or 2.8\%
of the maximum.
...
implemented using the techniques described in \S\ref{sec:LinearMixed}
and \S\ref{sec:Symbolic}.
...
\citet{lind:bate:1990} is essentially the PQL algorithm with the PIRLS
...
special circumstances.  Again, we use the techniques described in
\S\ref{sec:LinearMixed} and \S\ref{sec:Symbolic} in the PQL step and
in determining the conditional modes of the random effects for the
Laplace and adaptive Gauss-Hermite methods.
...
in the \code{lme4} package for R~\cite{R-2.0.0}.  In future versions
...
with partially crossed grouping factors to data sets with $10^6$ or
...
and, in passing, an implementation of the specification,
representation, and computational methods.  Furthermore, we show that
the specification, representation, methods, and implementation can be
extended to generalizations of the linear mixed model including GLMMs
and NMMs.

In many implementations of linear mixed models it can be awkward to
specify the structure of the random effects.  We contend that by
concentrating on the grouping factors in the data and model matrices
associated with these grouping factors, the specification of linear
mixed models is made much easier.  Nesting or crossing of grouping
factors can be determined from the data, rather than having to be
specified as part of the model.  Common forms of linear mixed models
(certainly all those in the comparative review of multilevel modeling
software) can be specified without using specialized forms of the
precision matrices $\bOmega_i, i=1,\dots,k$.  We only require these
matrices to be symmetric, positive semidefinite.  The structure and
interaction of the random effects is determined by the grouping
factors, not by esoteric forms of precision matrices (or
variance-covariance matrices) for the random effects.

A pairwise crosstabulation of the grouping factors, which we store as
a sparse symmetric matrix, is the basis for all the symbolic
analysis.  If we order the grouping factors so that $m1\ge
m2\ge\dots\ge m_k$ we can ``project out'' the 
