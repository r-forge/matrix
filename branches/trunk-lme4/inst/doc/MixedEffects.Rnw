...
\begin{abstract}
  ...
  off-diagonal elements is determined, and a numeric phase in which
  ...
  fill-minimizing permutation of the rows and columns of sparse
  semi-definite matrices.  These matrices have a special structure in
  the linear mixed-effects problem and we provide a new
  fill-minimizing algorithm tuned to this structure.
...
$\bZ\trans\bZ$ and $\bOmega$) can be very large, it can be divided
into components with simple structure.  
the association
...
\subsection{Criteria to evaluate}
...
Sparsity of $\bZ\trans\bZ$ and $\bOmega$ is induced by 
