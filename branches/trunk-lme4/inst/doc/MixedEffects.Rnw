...
$\bZ$, we wish to determine the values of the parameters $\btheta$,
$\bbeta$, and $\sigma^2$ that maximize either the likelihood or the
restricted likelihood. Because the conditional estimates of $\bbeta$
and $\sigma^2$, given a value of $\btheta$, can be determined from a
penalized least squares problem we can reduce the optimization problem
to one involving $\btheta$ only.  This reduction of the dimension of
the optimization problem is called \emph{profiling}.
...
elements of $\RZZ$ and $\RXX$ are not uniquely determined, $|RZZ|$ and
$\|\RXX\|$ are not well-defined but their magnitudes are
well-defined.)  The corresponding vectors, $\rZy$ and $\rXy$, are of
dimension $q$ and $p$, and $\ryy$ is a scalar.  The conditions that
$\bOmega$ be positive definite and $\bX$ have full column rank ensure
that $\RZZ$ and $\RXX$ are nonsingular.
...
throughout the parameter space and, furthermore, that the behavior of
these criteria at the extremes of the parameter space can be
characterized.
...
vector $\btheta$ where $\theta_i=\log\omega_i,i=1,\dots,k$ can be used
as the unconstrained parameter vector.
...
Because each block is a set of indicators, the diagonal
blocks of $\bZ\trans\bZ$ are diagonal.  However, unlike the
corresponding blocks in $\bOmega$, these blocks are not necessarily a
multiple of the identity.  The diagonal elements of the $i$th
diagonal block are the $m_i$ frequencies of occurence of each the
levels of the $i$th grouping factor in the data.  (Because all the
elements of $\bZ$ are zero or one, the diagonals of $\bZ\trans\bZ$ are
simply the counts of the number of ones in the corresponding column of
$\bZ$.)

The off-diagonal blocks of $\bZ\trans\bZ$ for the variance components
...
containing the achievement scores (\var{attain}), some demographic
data (\var{sex} and \var{social} class), a \var{verbal} reasoning
score based on tests taken at entry to secondary school, and the
\var{primary} and secondary (\var{second}) schools attended by 3435
students.  
...
have attended different secondary schools.
<<ScotsSecsummary>>=
...
secondary school we would say that \var{primary} is \emph{nested
  within} \var{second}.  That is no the case here.  There is a
moderate amount of \emph{crossing} of these two grouping factors.
...
print(image(as(fm1@rep, "sscMatrix"), aspect='xy', colorkey = FALSE))
@   
...
unity, they do not need to be stored explicitly.

The number of nonzero offdiagonal elements in $\bL$ depends on the
order of the rows and columns in $\bZ\trans\bZ+\bOmega$ because of
what is described as ``fill-in'' during the course of the
decomposition algorithm.  There are several methods available for
...
Metis~citep{Metis}.  However, the fill-reducing permutation from Metis
does not preserve separation of the grouping factors.  That is, the
permutation returned by Metis can, and usually does, intermingle the
rows and columns associated with different grouping factors.

To avoid this undesirable intermingling of the grouping factors, we
take the permutation returned by Metis and reorder it to separate the
factors.  We have not encountered any situations where this generates
substantial extra fill-in.
...
print(image(as(fm1@rep, "tscMatrix"),ylim=c(167,149),
            colorkey=FALSE,aspect='xy'))
@   
  \caption{Location of non-zero elements in the last 19 rows of
    $\bL$ for model \var{fm1} fit to the \var{ScotsSec} data.}
  \label{fig:Lnonzero}
\end{figure}

If the grouping factors are a nested sequence of factors there will
not be any fill-in.  In fact, the inverse of $\bL$ will have exactly
the same pattern of nonzeros as does $\bL$.  We check for a nested
sequence of grouping factors before seeking a fill-reducing
permutation.  The case $k=1$ (the random effects are determined by a
single grouping factor) is trivially a nested sequence.
...
in $\bL$.  The positions of the nonzeros in the last 19 rows of
$\bL^{-1}$ (again, the first 148 rows are an identity matrix augmented
with 19 columns of zeros) are shown in Figure~\ref{fig:LInonz}.
\begin{figure}[htbp]
  \centering
\setkeys{Gin}{width=\textwidth}
...
print(image(LI, ylim=c(167,149),
            colorkey=FALSE,aspect='xy'))
@   
  \caption{Location of non-zero elements in the last 19 rows of
    $\bL^{-1}$ for model \var{fm1} fit to the \var{ScotsSec} data.}
  \label{fig:LInonz}
\end{figure}
...
I would like to thank Deepayan Sarkar and Tim Davis for helpful
discussions and suggestions.  I also would like to thank Harold Doran
for his suggestion of using sparse matrix techniques with linear
