...
\begin{abstract}
  ...
  the rows and columns of the sparse semidefinite matrix.  This matrix
...
pattern of non-zeros in the $(i,j)$ block of $\bZ\trans\bZ$ is
obtained by replacing each non-zero in the $(i,j)$ block of the
crosstabulation by a $q_i\times q_j$ matrix.  Notice that we can
determine the patterns of non-zeros in $\bZ\trans\bZ$ knowing only the
...
grouping factors we record as potentially non-zero the positions $(i,k)$
with $i,k>m_1$ and both $(i,j)$ and $(k,j)$ non-zero for some
...
decomposition of this block in the original order for the rows and
...
However, in other cases the savings in the amount of storage required
for this block of $\bL$, and in the number of floating point
operations in solving linear systems of equations based on $\bL$, can
be substantial.  We have used these methods to analyze 378,047 test
scores of 134,713 students in 3722 schools.  There are 377,111 nonredundant
elements in the pairwise crosstabulation (138435 diagonals and
238,676 below the diagonal).  The projection of the $(2,1)$ block
into the $(2,2)$ block produces a symmetric $3722\times 3722$ matrix
with 49,305 nonredundant elements (3722 diagonals and 45,583 below the
diagonals).   Without the fill-reducing permutation the $\bL$
matrix has 4,469,124 off-diagonal elements or 64.5\% of the maximum
possible number (6,924,781). With the permutation the
number of off-diagonals is reduced to 193,562 (2.8\% of the maximum).
...
for GLMMs and the Laplace and adaptive Gauss-Hermite methods for NMMs.


\section{Conclusions}
\label{sec:Conclusions}

As shown in \citet{bate:debr:2004} the profiled log-likelihood or
log-restricted-likelihood for a linear mixed model can be conveniently
calculated from the solution to a penalized least squares problem.
Furthermore, the calculations for the conditional estimates of the
fixed-effects and variance parameters, the conditional modes of the
random effects, ECME iterations, and the gradient and Hessian of the
profiled objective function can all be expressed using these
solutions.  

In this paper we have described a 
a representation of linear mixed models where a sparse symmetric
matrix and several dense matrices are generated from grouping
factors and the model matrices associated with the random effects and
the model matrix for the fixed effects.
symmetric sparse matrices a
...
crosstabulations of the grouping factors and the $q_i,i=1,\dots,k$.
