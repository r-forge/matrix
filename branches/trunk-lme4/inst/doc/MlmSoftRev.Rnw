...
data(Exam)
str(Exam)
system.time(mE1 <- lme(normexam ~ standlrt + gender + schgend,
                       Exam, ~ 1|school), gc = TRUE)
summary(mE1)
system.time(mE2 <- lme(normexam ~ standlrt * gender + schgend, 
                       Exam, ~ 1|school), gc = TRUE)
summary(mE2)
system.time(mE3 <- lme(normexam ~ standlrt * gender + schgend, 
...
<<ExamIds>>=
Exam$ids <- (Exam$school : Exam$student)[, drop = TRUE]
str(Exam)
@ 
...
data(Chem97)
str(Chem97)
system.time(mC1 <- lme(score ~ 1, Chem97, ~ 1|lea/school))
summary(mC1)
system.time(mC2 <- lme(score ~ gcsecnt, Chem97, ~ 1|lea/school))
summary(mC2)
@ 
...
data(Oxboys)
str(Oxboys)
system.time(mX1 <- lme(height ~ age + I(age^2) + I(age^3) + I(age^4),
                       Oxboys, ~ age + I(age^2)|Subject), gc = TRUE)
summary(mX1)
system.time(mX2 <- lme(height ~ poly(age,4), Oxboys, ~ age + I(age^2)|Subject),
            gc = TRUE)
summary(mX2)
@ 
...
data(ScotsSec)
str(ScotsSec)
system.time(mS1 <- lme(attain ~ sex, ScotsSec, ~ 1|primary+second))
summary(mS1)
@ 
