...
#define BAND(c) \
	do { \
		...
		if (ge && class[1] != 'g') { \
		if (!packed) \
			c##NAME(force2)(px1, px0, n_, ul0, ct0, di0); \
		else \
			c##NAME( pack1)(px1, px0, n_, ul0, ct0, di0); \
		px0 = NULL; \
		packed = 0; \
...
#define FORCE(c) \
		do { \
			...
			if (!packed) \
				...
				c##NAME(force1)(px1,  px0, n_, ul0, ct0, di0); \
			else { \
				c##NAME(trans1)(px1,  px0, n_, ul0, ct0); \
				c##NAME(force1)(px1, NULL, n_, ul1, ct0, di0); \
			} \
		} while (0)
