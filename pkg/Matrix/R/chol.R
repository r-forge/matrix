...
## FIXME: this assumes that 'from' is packed and monotonic
setAs("dCHMsimpl", "dtCMatrix",
      function(from) {
          to <- new("dtCMatrix")
          to@Dim  <- from@Dim
...
setAs("dCHMsuper", "dgCMatrix",
      function(from) {
          ...
          to@Dimnames <- from@Dimnames
          to@p <- c(0L, cumsum(dp))
          to@i <- from@s[sequence.default(dp, rep.int(pi[-b], nc))]
          to@x <- from@x
          to
      })
...
setMethod("expand2", signature(x = "dCHMsimpl"),
          function(x, LDL = TRUE, ...) {
              d <- x@Dim
              dn <- x@Dimnames
              perm <- x@perm
...
## returning list(P1', L, L', P1) or list(P1', L1, D, L1', P1),
## where  A = P1' L L' P1 = P1' L1 D L1' P1  and  L = L1 sqrt(D)
setMethod("expand2", signature(x = "dCHMsuper"),
          function(x, LDL = TRUE, ...) {
              d <- x@Dim
              dn <- x@Dimnames
              perm <- x@perm
