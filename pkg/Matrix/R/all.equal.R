...
function(target, current,
    ...
    msg <- msg. <- NULL
    if (check.type  &&
        !identical(t1 <- typeof(target), t2 <- typeof(current)))
        msg <- c(msg, paste0("typeof(target) is ", deparse(t1), ", typeof(current) is ", deparse(t2)))
    if (check.class &&
        !identical(c1 <-  class(target), c2 <-  class(current)))
...
setMethod("all.equal",
          c(target = "Matrix", current = "vector"),
          .M.all.equal)

setMethod("all.equal",
          c(target = "vector", current = "Matrix"),
          .M.all.equal)

setMethod("all.equal",
          c(target = "Matrix", current = "Matrix"),
          .M.all.equal)

## And for completeness:

setMethod("all.equal",
          c(target = "Matrix", current = "sparseVector"),
          .M.all.equal)
...
setMethod("all.equal",
          c(target = "sparseVector", current = "vector"),
          .V.all.equal)

setMethod("all.equal",
          c(target = "vector", current = "sparseVector"),
          .V.all.equal)

setMethod("all.equal",
          c(target = "sparseVector", current = "sparseVector"),
          .V.all.equal)

## And for completeness:

setMethod("all.equal",
          c(target = "sparseVector", current = "Matrix"),
          .V.all.equal)
