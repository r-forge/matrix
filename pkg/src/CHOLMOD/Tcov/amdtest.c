...
/* -----------------------------------------------------------------------------
 * CHOLMOD/Tcov Module.  Version 0.6.  Copyright (C) 2005, Timothy A. Davis
...
void amdtest (cholmod_sparse *A)
{
	...
	*Len, *Nv, *Next, *Head, *Elen, *Deg, *Wi ;
    cholmod_sparse *C, *B, *S, *E, *F ;
    Int i, j, n, nrow, ncol, ok, cnz, bnz, p, trial ;
    ...
    Sp = S->p ;
    Si = S->i ;
    ok = AMD_preprocess (n, Bp, Bi, Sp, Si) ;
    ...
    Head = CHOLMOD(malloc) (n, sizeof (Int), cm) ;
    ...
    if (n > 0)
    {
	AMD_2 (n, Ep, Ei, Len, E->nzmax, Ep [n], Nv, Next, P, Head, Elen, Deg,
		Wi, Control, Info) ;
	AMD_info (Info) ;
    ...
    ok = AMD_order (n, Cp, Ci, P, Control, Info) ;	OK (ok == AMD_OK) ;
    ok = AMD_order (-1, Cp, Ci, P, Control, Info) ;	OK (ok == AMD_INVALID) ;
    ok = AMD_order (0, Cp, Ci, P, Control, Info) ;	OK (ok == AMD_OK) ;
    ok = AMD_order (n, NULL, Ci, P, Control, Info) ;	OK (ok == AMD_INVALID) ;
    ok = AMD_order (n, Cp, NULL, P, Control, Info) ;	OK (ok == AMD_INVALID) ;
    ok = AMD_order (n, Cp, Ci, NULL, Control, Info) ;	OK (ok == AMD_INVALID) ;
    ...
    ok = AMD_preprocess (n, Bp, Bi, Sp, Si) ;	    OK (ok == AMD_OK) ;
    ok = AMD_valid (n, n, Sp, Si) ;		    OK (ok) ;
    ok = AMD_valid (-1, n, Sp, Si) ;		    OK (!ok) ;
    ok = AMD_valid (n, -1, Sp, Si) ;		    OK (!ok) ;
    ok = AMD_valid (n, n, NULL, Si) ;		    OK (!ok) ;
    ok = AMD_valid (n, n, Sp, NULL) ;		    OK (!ok) ;
    ...
    ok = AMD_preprocess (-1, Bp, Bi, Sp, Si) ;	    OK (ok == AMD_INVALID) ;
    ok = AMD_preprocess (n, NULL, Bi, Sp, Si) ;	    OK (ok == AMD_INVALID) ;
    ok = AMD_preprocess (n, Bp, NULL, Sp, Si) ;	    OK (ok == AMD_INVALID) ;
    ok = AMD_preprocess (n, Bp, Bi, NULL, Si) ;	    OK (ok == AMD_INVALID) ;
    ok = AMD_preprocess (n, Bp, Bi, Sp, NULL) ;	    OK (ok == AMD_INVALID) ;
    ...
    CHOLMOD(free) (n, sizeof (Int), Head, cm) ;
