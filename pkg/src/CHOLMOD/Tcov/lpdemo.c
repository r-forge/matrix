...
double lpdemo (cholmod_triplet *T)
{
	...
	*Sp, *Si, *StaticParent ;
	...
	rowmark = CHOLMOD(malloc) (nrow, sizeof (Int), cm) ;
	...
	if (rowmark != NULL)
	{
	    ...
	    /* update L, and the solution to Lx=b+deltaB,
	     * but only update solution in rows 0 to nrow/2 */
	    C = lp_prune (A, rflag, cols, 1) ;
	    ok = CHOLMOD(updown_mark) (TRUE, C, colmark, L, X, DeltaB, rowmark,
		    cm) ;
	    CHOLMOD(free_sparse) (&C, cm) ;

	    /* compare with Lr=b+deltaB */
	    R = CHOLMOD(solve) (CHOLMOD_L, L, B, cm) ;
	    if (ok && R != NULL)
	    {
		double *Rx ;
		Rx = R->x ;
	...
	CHOLMOD(free) (nrow, sizeof (Int), rowmark, cm) ;
    }
