...
SEXP lmer_factor(SEXP x)
{
	...
	for (i = 0; i < nf; i++) {
		...
		/* FIXME: This should be in L, not ZZpO or change L to diagonal blocks*/
		cscb_trcbm('R', 'L', 'T', 'U', 1.0, LinvP, ZZOji);
		for (jj = 0; jj < nlev; jj++) {
		    int k, k2 = ZZp[jj + 1];
		    for (k = ZZp[jj]; k < k2; k++)
			F77_CALL(dtrsm)("R", "U", "N", "N", &nci, xdims + 1,
					&one, D + jj * ncisqr, &nci,
					ZZO + k * ntot, xdims);
		}
		...
		for (jj = 0; jj < nlev; jj++) {
		    int k, k2 = ZZp[jj + 1];
		    for (k = ZZp[jj]; k < k2; k++)
			F77_CALL(dtrsm)("R", "U", "T", "N", &nci, xdims + 1,
					&one, D + jj * ncisqr, &nci,
					ZZO + k * ntot, xdims);
		}
	    }
