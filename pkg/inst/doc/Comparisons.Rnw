...
system.time(t(mm) %*% mm)
@
Note that in newer versions of \RR{} and the LAPACK library (as of summer 2007),
\RR's \code{\%*\%} is able to detect the many zeros in \code{mm} and shortcut
many operations, and is hence much faster for such a sparse matrix
than \code{crossprod} which currently does not make use of such optimizations.
For really dense matrices however, \code{crossprod} indeed remains faster
(by a factor of two on one typical platform):
...
all.equal(naive.sol, unname(as(sparse.sol, "matrix")))
@
