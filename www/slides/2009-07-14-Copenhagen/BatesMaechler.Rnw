...
## Hide this as well
load("~/R/MM/Pkg-ex/lme4/eth-d-rating.rda") ; d.eth <- md
@
...
\begin{frame}[fragile]\frametitle{'d' as random effect: Regularization}
  ...
  Adantage: Can easily use \texttt{dept} as a fixed effect:

%%% FIXME:  Need "old" lme4 for this!
<<change-to-old-lme4, echo=FALSE, hide=FALSE>>=
detach("package:lme4a")
library(lme4)
@

% <<lmer-eth-rf, fig=TRUE>>=
...
load(file="lmer-eth.rda") # -> beta.fix, ranef.fm2, fixef.fm2
@
...
\begin{frame}[fragile]
<<lmer-eth-plot-fix-rf, fig=TRUE>>=
plot(b.random ~ beta.Fix, main = "shrinkage of random effects")
